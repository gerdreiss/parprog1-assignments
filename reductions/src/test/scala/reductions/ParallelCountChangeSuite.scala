...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("parCountChange should return 0 for money < 0") {
    def check(money: Int, coins: List[Int]) =
      assert(parCountChange(money, coins,
        ParallelCountChange.combinedThreshold(money, coins)) == 0,
        s"parCountChange($money, _, _) should be 0")
  ...
  test("parCountChange should return 1 when money == 0") {
    def check(coins: List[Int]) =
      assert(parCountChange(0, coins,
        ParallelCountChange.combinedThreshold(0, coins)) == 1,
        s"parCountChange(0, _, _) should be 1")
  ...
  test("parCountChange should return 0 for money > 0 and coins = List()") {
    def check(money: Int) =
      assert(parCountChange(money, List(),
        ParallelCountChange.combinedThreshold(money, List())) == 0,
        s"parCountChange($money, List(), _) should be 0")
  ...
  test("parCountChange should work when there is only one coin") {
    def check(money: Int, coins: List[Int], expected: Int) =
      assert(parCountChange(money, coins,
        ...
        s"parCountChange($money, $coins, _) should be $expected")
